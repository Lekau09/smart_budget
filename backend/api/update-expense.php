...
try {
  ...
  // Also update transactions table if it exists — ignore errors since schema may differ
  try {
    $check_transactions = $conn->prepare("SELECT 1 FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'transactions'");
    $check_transactions->execute();
    if ($check_transactions->get_result()->num_rows > 0) {
      $update_transaction = $conn->prepare("
        UPDATE transactions
        SET description = ?, amount = ?, category = ?, date = ?
        WHERE id = ? AND user_id = ?
      ");
      $update_transaction->bind_param("sdssii", $description, $amount, $category, $date, $expense_id, $user_id);
      $update_transaction->execute();
      $update_transaction->close();
    }
    $check_transactions->close();
  } catch (Exception $e) {
    // transactions table may have different schema — not critical
  }
